...
function PureAttachmentsButton({
  ...
  const { data: caps } = useSWR<Record<string, ModelCapabilities>>(
...
function ModelSelectorOption({
  ...
  const withTooltip = (icon: ReactNode, label: string) => (
    <Tooltip>
      <TooltipTrigger asChild>
        <span className="inline-flex rounded-md bg-muted/60 p-1 text-foreground/70">
          {icon}
        </span>
      </TooltipTrigger>
  ...
  return (
    <ModelSelectorItem
      className={cn(
        "flex w-full items-center gap-2 rounded-lg px-2 py-2 transition-colors data-[selected=true]:bg-muted data-[selected=true]:text-foreground",
        isSelected && "bg-accent/40 font-medium ring-1 ring-inset ring-foreground/15"
      )}
    ...
    >
      ...
      {isSelected && (
        <span className="ml-1 size-1.5 rounded-full bg-foreground" />
      )}
